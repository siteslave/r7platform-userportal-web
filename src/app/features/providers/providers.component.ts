...
export class ProvidersComponent {
  ...
  @ViewChild('mdlNew') private mdlNew!: ModalProviderNewComponent;
  ...
  constructor (
    ...
    private nationService: ProviderService,
    private message: NzMessageService
  ) {
    const token = sessionStorage.getItem('token')
    this.uploadHeader = { authorization: 'Bearer ' + token }
    this.uploadUrl = `${this.apiUrl}/libs/providers/upload`
  }
  ...
  async getItems() {
    ...
    try {
      ...
      this.datasets = responseData.data.map((v: any) => {
        v.created_at = DateTime.fromISO(v.created_at, {
          zone: "Asia/Bangkok",
          locale: 'th'
        }).toLocaleString(DateTime.DATETIME_SHORT)

        v.updated_at = DateTime.fromISO(v.updated_at, {
          zone: "Asia/Bangkok",
          locale: 'th'
        }).toLocaleString(DateTime.DATETIME_SHORT)
  ...
  editItem(item: IProvider) {
    this.mdlNew.showModalEdit(item)
  }
